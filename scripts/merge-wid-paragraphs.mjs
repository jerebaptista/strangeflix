...
const p = path.join(__dirname, "../content/books/the-whisperer-in-darkness.txt");
let raw = fs.readFileSync(p, "utf8");
const header = raw.match(/^The Whisperer in Darkness\nBy H\. P\. Lovecraft\n\n/)[0];
...
for (const b of blocks) {
  if (/^(I{1,3}|IV|V|VI|VII|VIII)\.$/.test(b)) {
    if (cur) {
      out.push(cur);
      cur = "";
    }
