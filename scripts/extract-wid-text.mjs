...
const htmlPath = path.join(root, "content/books/_wid.html");
const outPath = path.join(root, "content/books/the-whisperer-in-darkness.txt");

const html = fs.readFileSync(htmlPath, "utf8");
const start = html.indexOf("<br /><center>I.</center>");
const endMarker = "Henry Wentworth Akeley.<br />";
...
const lines = chunk
  ...
  .map((l) => l.trim())
  .filter((l) => l.length > 0);
const title = "The Whisperer in Darkness\nBy H. P. Lovecraft\n\n";
