...
const htmlPath = path.join(root, "content/books/_sot.html");
const outPath = path.join(root, "content/books/the-shadow-out-of-time.txt");

const html = fs.readFileSync(htmlPath, "utf8");
const start = html.indexOf("<br /><center>I.</center>");
const endMarker = "English language in my own handwriting.<br />";
const end = html.indexOf(endMarker);
if (start < 0 || end < 0) throw new Error("Could not find story bounds");
let chunk = html.slice(start, end + endMarker.length);
...
const lines = chunk
  ...
  .map((l) => l.trim())
  .filter((l) => l.length > 0);
const title = "The Shadow out of Time\nBy H. P. Lovecraft\n\n";
